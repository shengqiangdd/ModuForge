...
curl -s -X POST "http://localhost:8086/api/v1/llm/config" \
  ...
  -d '{{"provider":"opencode-zen","model_id":"nemotron-3-ultra-free"}}'
""")
print(stdout.read().decode('utf-8', errors='replace'))

# 3. Get first project
stdin, stdout, stderr = ssh.exec_command(f"""
curl -s http://localhost:8086/api/v1/projects -H "Authorization: Bearer {token}" | python3 -c "import sys,json; ps=json.load(sys.stdin).get('projects',[]); print(ps[0]['id'] if ps else '')"
""")
pid = stdout.read().decode('utf-8', errors='replace').strip()
print(f'Project: {pid}')

# 4. Test agent with nemotron-3-ultra-free (longer timeout)
print('\n=== Test agent with nemotron-3-ultra-free ===')
stdin, stdout, stderr = ssh.exec_command(f"""
curl -s -X POST "http://localhost:8086/api/v1/agent/run" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer {token}" \
  -d '{{"task":"List all files in this project using the list_dir tool with path \\".\\"","project_id":"{pid}","agent_mode":"act","provider_id":"opencode-zen","model":"nemotron-3-ultra-free"}}' \
  -m 120 2>&1 | python3 -c "
import sys, json
for line in sys.stdin:
    ...
    try:
        ...
        t = d.get('type', '')
        if t == 'step':
            print(f'  [{d.get(\"step\",\"?\")}] {d.get(\"content\",\"\")}')
        elif t == 'tool':
            print(f'  [TOOL] {d.get(\"name\",\"?\")}({d.get(\"args\",{{}})})')
        elif t == 'tool_result':
            ...
            print(f'  [FINAL] {d.get(\"content\",\"\")[:500]}')
        else:
            pass
    except:
        pass

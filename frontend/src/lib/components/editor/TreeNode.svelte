<script lang="ts">
  ...
  // IMPORTANT: query the Set directly instead of relying on isExpanded prop.
  // `isExpanded ?? ...` short-circuits when isExpanded is a boolean (false),
  // so the Set is never consulted and folders never expand.
  let dirExpanded = $derived(expandedDirs.has(node.path));

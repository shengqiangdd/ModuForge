...
<div class="code-analyzer">
  <h2 class="text-xl font-semibold mb-4">代码分析</h2>
  ...
  {#if result?.valid && result.result}
    ...
    <div class="card mb-4">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
        <div>
          <div class="text-gray-500">包名</div>
          <div class="font-semibold">{result.result.package_name}</div>
        </div>
        <div>
        ...
        <div>
          <div class="text-gray-500">总复杂度</div>
          <div class="font-semibold {getComplexityColor(result.result.complexity)}">
            {result.result.complexity}
          </div>
        </div>
        <div>
          <div class="text-gray-500">导入数</div>
          <div class="font-semibold">{result.result.imports?.length || 0}</div>
        </div>
      </div>
    </div>
    ...
    {#if result.result.functions?.length > 0}
      <div class="card mb-4">
        <h3 class="card-title">函数列表 ({result.result.functions.length})</h3>
          ...
          {#each result.result.functions as func}
            <div class="p-2 bg-gray-50 rounded flex items-center justify-between">
              <div>
                ...
                {#if func.receiver}
                  <span class="text-gray-400 text-xs">({func.receiver})</span>
                {/if}
              </div>
              <div class="flex items-center gap-4 text-xs text-gray-500">
                <span>复杂度: <span class="{getComplexityColor(func.complexity)}">{func.complexity}</span></span>
                <span>{func.exported ? '导出' : '私有'}</span>
              </div>
            </div>
          {/each}
        </div>
    ...
    <!-- 结构体列表 -->
    {#if result.result.structs?.length > 0}
      <div class="card mb-4">
        <h3 class="card-title">结构体 ({result.result.structs.length})</h3>
        <div class="space-y-2 max-h-48 overflow-y-auto">
          {#each result.result.structs as s}
            <div class="p-2 bg-gray-50 rounded">
              <div class="flex items-center justify-between">
                <span class="font-mono text-sm">{s.name}</span>
                <span class="text-xs text-gray-500">{s.fields?.length || 0} 字段, {s.methods?.length || 0} 方法</span>
              </div>
            </div>
          {/each}
        </div>
    ...
    {#if result.result.warnings?.length > 0}
      <div class="card">
        <h3 class="card-title">警告 ({result.result.warnings.length})</h3>
        <ul class="space-y-1">
  ...
  {:else if result && !result.valid}
    <div class="card text-red-500">
      <div class="font-medium">解析错误</div>

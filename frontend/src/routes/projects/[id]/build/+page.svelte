<script lang="ts">
  ...
  async function startBuild() {
    ...
    try {
      const res = await fetch(`/api/v1/projects/${id}/build`, {
        ...
        body: JSON.stringify({ target: 'universal' }),
      });
  ...
  const statusConfig: Record<string, { color: string; bg: string; icon: string; label: string }> = {
    pending: { color: 'text-[var(--color-warning)]', bg: 'bg-[var(--color-warning-light)]', icon: 'schedule', label: '等待中' },
    running: { color: 'text-[var(--color-info)]', bg: 'bg-[var(--color-info-light)]', icon: 'sync', label: '构建中' },
    success: { color: 'text-[var(--color-success)]', bg: 'bg-[var(--color-success-light)]', icon: 'check_circle', label: '构建成功' },
    failed: { color: 'text-[var(--color-error)]', bg: 'bg-[var(--color-error-light)]', icon: 'error', label: '构建失败' },
  };
</script>
...
<div class="p-6 max-w-3xl mx-auto">
  <div class="mb-8">
    ...
    <p class="text-sm text-[var(--color-text-secondary)] mt-0.5">一键构建兼容 Magisk / KernelSU / APatch 的通用模块</p>
  </div>

  <!-- Universal Badge -->
  <div class="mb-8 p-5 rounded-2xl border border-[var(--color-border)] bg-[var(--color-surface)]">
    <div class="flex items-center gap-4">
      ...
      <div>
        ...
        <p class="text-xs text-[var(--color-text-muted)] mt-0.5">自动兼容 Magisk · KernelSU · APatch 三种管理器</p>
      </div>
      <div class="ml-auto flex items-center gap-1 px-2.5 py-1 rounded-lg" style="background: var(--color-primary-light); color: var(--color-primary)">
        <span class="material-symbols-outlined text-[14px]">check_circle</span>
        <span class="text-[11px] font-medium">三端兼容</span>
      </div>
    </div>
  </div>
  ...
  {#if status}
    ...
    <div class="mt-6 card p-4 {cfg.bg}" style="border-color: var(--color-border)">

      ...
      {#each projects as project}
        <div class="card-hover p-5 group">
          <div class="flex items-center gap-3 mb-3">
            <div class="w-10 h-10 rounded-xl bg-primary-100 flex items-center justify-center group-hover:scale-105 transition-transform">
              <span class="material-symbols-outlined text-primary-600 text-xl">code_blocks</span>
            </div>
            <div>
              <h3 class="font-semibold text-[var(--color-text)]">{project.name}</h3>
              <span class="text-[10px] font-medium px-2 py-0.5 rounded-full bg-primary-50 text-primary-700">{project.module_type}</span>
            </div>
          </div>
          ...
          <div class="flex gap-2">
            ...
            <a href="/projects/{project.id}/build" class="flex-1 py-2 rounded-xl text-sm font-medium text-center no-underline border border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-neutral-50 transition-colors">构建</a>
          </div>
        </div>
      {/each}
    </div>

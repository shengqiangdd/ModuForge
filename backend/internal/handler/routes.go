...
func RegisterRoutes(api fiber.Router, db *database.DB, cfg *config.Config) {
	...
	// Services
	// Initialize S3-compatible storage (SeaweedFS/MinIO) if endpoint is configured.
	// S3 is the authoritative file-content source; DB keeps only metadata.
	...
	projectSvc := service.NewProjectService(db.Conn, cfg.StoragePath, s3adapter)
	...
	api.Get("/adb/devices", jwtMW, adbH.ListDevices)
	...
	r("GET", "/agent/mcp/servers", agentH.ListMCPServers)
	r("POST", "/agent/mcp/servers", agentH.AddMCPServer)
	...
	r("POST", "/agent/mcp/confirm", agentH.ConfirmMCPApproval)

...
func (r *AgentRunner) buildToolDefinitionsForMode(mode AgentMode, modelName string) []ToolDef {
	...
	for _, s := range skills {
		...
		case "syntax_checker":
			def.Function.Description = "Pre-build syntax validation for Go, Rust, C/C++, Shell scripts. Run AFTER writing code but BEFORE build_module to catch errors early. Returns structured errors with line numbers and fix hints."
			def.Function.Parameters = toolParams(map[string]interface{}{
				"project_id": map[string]interface{}{"type": "string", "description": "Project ID"},
				"language":   map[string]interface{}{"type": "string", "enum": []interface{}{"auto", "go", "rust", "cpp", "shell"}, "description": "Language to check (default: auto-detect)"},
			}, nil)
		case "test_module":
